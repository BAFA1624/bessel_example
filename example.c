...
{
	// Returns value of f(m, x, theta), theta is in 0th pos
	// as it is the variable the integration occurs over.
...
double *linspace(double start, double stop, int N)
{
	...
	if (!result)
	{
		fprintf(stderr, "Failed malloc in <linspace>");
		exit(-1);
	}
	if (stop < start)
	{
		fprintf(stderr, "Endpoint is before startpoint in <linspace>");
		exit(-1);
	}
...
double integrate(double (*func)(double *), double *params, double a, double b, int N)
{
	...
	if (b < a)
	{
		fprintf(stderr, "Endpoint is before startpoint in <integrate>");
	}
	double h = (b - a) / N;
